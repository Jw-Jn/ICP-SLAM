...
inline cv::Mat transformToMatrix(tf::Transform transform, float resolution)
{
  cv::Mat matrix(cv::Mat::eye(3, 3, CV_32F));
  matrix.at<float>(0, 2) = transform.getOrigin().getX()/resolution;
  matrix.at<float>(1, 2) = transform.getOrigin().getY()/resolution;
...
inline cv::Mat transformToTransformationMatrix(tf::Transform transform, float resolution)
{
  cv::Mat matrix(cv::Mat::eye(3, 3, CV_32F));
  matrix.at<float>(0, 2) = transform.getOrigin().getX()/resolution;
  matrix.at<float>(1, 2) = transform.getOrigin().getY()/resolution;
