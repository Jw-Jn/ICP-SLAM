...
ICPSlamNode::ICPSlamNode() : local_nh_("~")
{
  ...
  ROS_INFO("ODOM FRAME: %s", odom_frame_id_.c_str());
  ...
  local_nh_.param("xmin", x_min_, -50.0);
  local_nh_.param("ymin", y_min_, -50.0);
  local_nh_.param("xmax", x_max_, 50.0);
  local_nh_.param("ymax", y_max_, 50.0);
...
void ICPSlamNode::laserCallback(const sensor_msgs::LaserScanConstPtr &laser_msg)
{
  ...
  try {
    tf_listener_.waitForTransform("odom", "base_link", ros::Time(0), ros::Duration(1.0));
  ...
  // tf_odom_laser.getOrigin().getX(),
  // tf_odom_laser.getOrigin().getY(),
